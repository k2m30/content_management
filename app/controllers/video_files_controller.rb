class VideoFilesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @video_file.save
        ...
        format.json { render action: 'show', status: :created, location: @video_file }
      else
  ...
  def destroy
    @video_file.destroy
    respond_to do |format|
      format.html { redirect_to video_files_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def video_file_params
      params.require(:video_file).permit(:internal_name, :internal_url, :external_name, :external_url, :quality, :year, :size)
    end
end
